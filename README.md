...
* [`src/reframe_sign_in/`](src/reframe_sign_in/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframe_sign_in/core.cljs): contains the SPA entry point, `init`
* [`test/reframe_sign_in/`](test/reframe_sign_in/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `reframe-sign-in` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_sign_in/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-sign-in.example
  (:require [reframe-sign-in.config :as config])
